      ...
      try {
        ...
        if (linkRes.ok) {
          ...
          if (linkRes.status === 401) {
            console.warn('Guest data merge failed due to expired or invalid token. Continuing with registration.');
          } else {
            console.warn('Failed to merge guest data, but registration successful:', errorData.error || 'Unknown error');
          }
        }
      } catch (linkError) {
