...
Inbound messages are immediately acknowledged by the webhook and then queued for background processing. The system features a context-driven AI architecture that builds user-specific data packets from 30-day spending patterns, enforcing structured responses through JSON schema validation. When user context is insufficient (<2 categories), the system guards against generic advice and prompts for data collection ("log 3 biggest spends"). All AI responses follow a summary/action/question structure and are limited to 280 characters with graceful clipping. An RL-2 Graceful Non-AI Fallback System ensures deterministic processing for rate-limited scenarios.
