...
Inbound messages are immediately acknowledged by the webhook and then queued for background processing. The system features an engagement-driven AI architecture with proactive onboarding and personalized user interactions. New users receive a welcome sequence with income assessment, spending category identification, and focus area selection. The context-driven system builds user-specific data packets from 30-day spending patterns, enforcing structured responses through JSON schema validation. When user context is insufficient (<2 categories), the system guards against generic advice and prompts for data collection ("log 3 biggest spends"). All AI responses follow a summary/action/question structure and are limited to 280 characters with graceful clipping. A per-user AI rate limiter (4 requests/60s) prevents spam while maintaining engagement. Habit-forming UX elements trigger weekly challenges and check-ins after interaction thresholds.
